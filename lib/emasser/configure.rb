...
module Emasser::Configure
  ...
  class << self
    ...
    def configure
      ...
      # Optional variables
      answers['EMASSER_USER_UID'] = get_optional_variable('Provide the eMASS User unique identifier', default: ENV.fetch('EMASSER_USER_UID', nil))
    ...
    private def get_required_variable(question, default: nil)
      ...
      prompt.ask("#{question}?:".blue, default: ENV.fetch(default, nil), required: true, active_color: :yellow)
    end

    private def very_file_path_exist(question, default: nil)
      prompt = TTY::Prompt.new(quiet: false, help_color: :yellow, interrupt: :exit)
      prompt.ask("#{question}?:".blue, default: ENV.fetch(default, nil), required: true, active_color: :yellow, convert: :filepath) do |q|
    ...
    private def get_optional_mask_variable(question, default: nil)
      prompt = TTY::Prompt.new(quiet: false, help_color: :yellow, interrupt: :exit)
      prompt.mask("#{question}?:".blue, default: ENV.fetch(default, nil), active_color: :yellow, mask: '*')
    end
    ...
    # rubocop: disable Style/AccessModifierDeclarations, Metrics/PerceivedComplexity
    ...
    private def update_key_value_pairs(file_path, updates)
      ...
      puts "Error reading or writing file: #{e}".red
      exit(1)
    end
    # rubocop: enable Lint/DuplicateBranch
    # rubocop: enable Style/AccessModifierDeclarations, Metrics/PerceivedComplexity
  end
end
